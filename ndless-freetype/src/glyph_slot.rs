...
impl GlyphSlot {
	...
	/// Retrieve a description of a given subglyph. Only use it if the glyph's
	/// format is FT_GLYPH_FORMAT_COMPOSITE; an error is returned otherwise.
	...
	/// Returns a glyph object, that is similar to a `GlyphSlot` but managed
	...
	/// In freetype, the `Outline` object is a scalable glyph. This method
	/// unpacks a glyph into this object, or returns `None` if the glyph has no
	/// `outline`
	...
	/// buffer can change between calls of FT_Load_Glyph and a few other
	/// functions.
	#[inline(always)]
	...
	/// The bitmap's left bearing expressed in integer pixels. Only valid if the
	/// format is FT_GLYPH_FORMAT_BITMAP, this is, if the glyph slot contains a
	/// bitmap.
	#[inline(always)]
	...
	/// The bitmap's top bearing expressed in integer pixels. Remember that this
	/// is the distance from the baseline to the top-most glyph scanline,
	...
	/// pixel format. As specified with FT_LOAD_VERTICAL_LAYOUT, it uses either
	...
	/// glyph. This field can be important to perform correct WYSIWYG layout.
	...
	/// can be expressed either in 26.6 fractional pixels or font units.
	#[inline(always)]
